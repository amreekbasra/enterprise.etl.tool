...
{
    // public class SourceDbReadToElastic
    // {
...
class Program
{
    ...
    static async Task Main()
    {
        ...
        string elasticsearchUrl = "http://localhost:9200/my-index/_bulk"; // Change accordingly

        // Field mapping (SourceColumn, DestinationField, SourceType, DestinationType)
    ...
    static object ConvertValue(object? value, string sourceType, string destinationType)
    {
        try
        {
            if (value == DBNull.Value) return destinationType == "int" || destinationType == "float" || destinationType == "double" ? 0 : null!;
